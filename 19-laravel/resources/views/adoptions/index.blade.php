...
@section('content')
    ...
    <h1 class="mt-6 text-4xl text-white flex gap-2 items-center justify-center pb-4 border-b-2 border-neutral-50 mb-10">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-10" fill="currentColor" viewBox="0 0 256 256">
            <path d="M240,208H224V96a16,16,0,0,0-16-16H144V48a16,16,0,0,0-16-16H32A16,16,0,0,0,16,48V208H0a8,8,0,0,0,0,16H240a8,8,0,0,0,0-16ZM208,96V208H144V96ZM32,48H128V208H112V168a8,8,0,0,0-8-8H72a8,8,0,0,0-8,8v40H32Zm48,160V176h32v32Z"/>
        </svg>
        Module Adoptions
    </h1>
    ...
    <div class="flex flex-col gap-4 justify-center items-center">
        <div class="join mx-auto">
            <a class="btn btn-outline btn-success join-item" href="{{ url('adoptions/create') }}">
                ...
                <span class="hidden md:inline">New Adoption</span>
            </a>
        ...
        <label class="input text-white bg-[#0009] w-58 md:w-112 outline outline-white mb-10">
            ...
            <input type="search" placeholder="Search by user or pet..." name="qsearch" id="qsearch" />
        </label>
    </div>
            ...
            <tbody class="datalist">
                @foreach ($adoptions as $adoption)
                    <tr class="text-white even:bg-blue-900">
                        <td class="hidden md:table-cell">{{ $adoption->id }}</td>
                        <td>
                            <div class="avatar">
                        ...
                        <td class="hidden md:table-cell">{{ $adoption->pet->kind }}</td>
                        <td class="hidden md:table-cell">{{ $adoption->created_at->format('Y-m-d') }}</td>
                        <td class="flex gap-1 justify-center items-center h-20">
                            ...
                            <a href="javascript:;" class="btn btn-outline btnxs btn-error btn-delete"
                                data-name="{{ $adoption->user->fullname }} / {{ $adoption->pet->name }}">
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="currentColor" viewBox="0 0 256 256">
...
@section('js')
    <script>
        ...
        $('.btn-delete').click(function () {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "Adoption: " + $name + " will be deleted!",
                icon: "warning",
        ...
        $('body').on('input', '#qsearch', function (event) {
            ...
            $('.datalist').html(`<tr><td colspan="8" class="text-center py-18">
                <span class="loading loading-spinner loading-xl"></span></td></tr>`)
            if (query != '') {
                search(query)
            } else {
                setTimeout(() => { window.location.replace('adoptions') }, 500)
            }
        })
    </script>
@endsection